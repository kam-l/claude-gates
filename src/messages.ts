...
function fmt(emoji: string, text: string): string {
  return `${PREFIX} ${emoji} ${text}`;
}

/** Block a tool call. reason is injected into Claude's context. */
function block(emoji: string, text: string): void {
...
function info(emoji: string, text: string): void {
  const msg = fmt(emoji, text);
...
function notify(sessionDir: string, emoji: string, text: string): void {
  const msg = fmt(emoji, text);
...
function drainNotifications(sessionDir: string): string | null {
  const filePath = path.join(sessionDir, ".pipeline-notifications");
  try {
...
function log(emoji: string, text: string): void {
  process.stderr.write(fmt(emoji, text) + "\n");
}

export { fmt, block, info, notify, drainNotifications, log };
