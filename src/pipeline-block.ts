...
import { VERDICT_RE, getSessionDir, agentRunningMarker } from "./pipeline-shared";
...
try {
  ...
  try {
    ...
    if (actions && actions.length > 0) {
      for (const act of actions) {
        try {
          const row = db.prepare("SELECT trace_id FROM pipeline_state WHERE scope = ?").get(act.scope) as any;
  ...
  const commandAllowedTools = new Set<string>();
