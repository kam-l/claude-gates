...
fs.writeFileSync(
  path.join(agentsDir, "e2e-builder.md",),
  `---
name: e2e-builder
...
fs.writeFileSync(
  path.join(agentsDir, "e2e-reviewer.md",),
  `---
name: e2e-reviewer
...
fs.writeFileSync(
  path.join(agentsDir, "e2e-conditional.md",),
  `---
...
test("full flow: parse agent → create pipeline → step through", () =>
{
  ...
  assert.strictEqual(steps![1].type, "VERIFY",);
...
test("full flow: role resolution through pipeline lifecycle", () =>
{
  ...
  try
  {
    ...
    assert.strictEqual(engine.resolveRole(db, "role-scope", "e2e-reviewer",), "ungated",); // step 0 is CHECK, not VERIFY
...
{
  const steps: any[] = [{ type: "VERIFY", agent: "rev", maxRounds: 3, },];
  const dir = tmpDir();
  const db = crud.getDb(dir,);
  try
  {
...
test("source REVISE advances CHECK (source doesn't judge itself)", () =>
{
  ...
  try
  {
    ...
    const a: any = engine.step(db, "decouple-scope", { role: "source", artifactVerdict: "PASS", },);
    assert.strictEqual(a.action, "spawn", "Should advance to VERIFY step",);
...
test("source FAIL still treated as PASS when semantic is null (source doesn't self-judge)", () =>
{
  ...
  try
  {
    ...
    const a: any = engine.step(db, "decouple-fail", { role: "source", artifactVerdict: "PASS", },);
...
test("semantic FAIL still triggers revision even when source says PASS", () =>
{
  ...
  try
  {
    ...
    const a: any = engine.step(db, "sem-override", { role: "source", artifactVerdict: "FAIL", },);
...
test("gater as VERIFY agent: engine.step advances (no short-circuit)", () =>
{
  ...
  try
  {
    ...
    const a: any = engine.step(db, "gater-scope", { role: "verifier", artifactVerdict: "PASS", },);
...
test("gater as VERIFY agent: REVISE routes back to source", () =>
{
  ...
  try
  {
    ...
    const a: any = engine.step(db, "gater-rev", { role: "verifier", artifactVerdict: "REVISE", },);
    assert.strictEqual(a.action, "source",);
...
test("gt-worker verification: field parses to CHECK + VERIFY_W_FIXER", () =>
{
  const agentMd = fs.readFileSync(
    path.join(process.cwd(), ".claude", "agents", "gt-worker.md",),
    "utf-8",
  );
  const steps = shared.parseVerification(agentMd,);
  assert.ok(steps, "parseVerification should return steps",);
  ...
  assert.strictEqual(steps![1].type, "VERIFY_W_FIXER",);
...
{
  const agentMd = fs.readFileSync(
    path.join(process.cwd(), ".claude", "agents", "gt-reviewer.md",),
    "utf-8",
  );
  const steps = shared.parseVerification(agentMd,);
