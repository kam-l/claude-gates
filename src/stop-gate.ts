...
import { loadConfig } from "./claude-gates-config";
...
try {
  ...
  try {
    ...
    if (enabled) {
      const allPipelines = db.prepare("SELECT scope, status, trace_id FROM pipeline_state").all() as any[];
  ...
  const PATTERNS = (config.stop_gate.patterns || []).map((p: string) => ({
    name: p,
