...
function discoverSessions(): string[]
{
  try
  {
    return fs.readdirSync(SESSIONS_DIR,)
      .filter((entry,) => SESSION_DIR_RE.test(entry,) && fs.existsSync(path.join(SESSIONS_DIR, entry, "session.db",),));
  }
  catch
  {
    return [];
...
function openReadonly(sessionId: string,): any | null
{
  if (!Database)
  {
    return null;
...
function withDb<T,>(sessionId: string, fn: (db: any,) => T,): T | null
{
...
function listScopeFiles(sessionId: string, scope: string,): string[]
{
  const scopeDir = path.join(SESSIONS_DIR, sessionId, scope,);
  try
  {
    return fs.readdirSync(scopeDir,).filter((f,) => f.endsWith(".md",));
  }
  catch
  {
    return [];
...
function readArtifact(sessionId: string, scope: string, filename: string,): string | null
{
  const filePath = path.join(SESSIONS_DIR, sessionId, scope, filename,);
  try
  {
    ...
    const data = withDb(id, (db,) =>
    {
      try
      {
        const pipelines = db.prepare(
          "SELECT scope, source_agent, status, current_step, total_steps, created_at FROM pipeline_state ORDER BY created_at DESC",
        ).all();
        return { id, pipelines, };
      }
      catch
...
function apiSession(sessionId: string,): object | null
{
  ...
  return withDb(sessionId, (db,) =>
  {
    ...
    {
      const pipelines = db.prepare(
        "SELECT scope, source_agent, status, current_step, total_steps, trace_id, created_at FROM pipeline_state ORDER BY created_at DESC",
      ).all();

      for (const p of pipelines as any[])
      {
        p.steps = db.prepare(
          "SELECT step_index, step_type, prompt, agent, max_rounds, fixer, status, round, source_agent FROM pipeline_steps WHERE scope = ? ORDER BY step_index",
        ).all(p.scope,);
        p.agents = db.prepare(
          "SELECT agent, outputFilepath, verdict, \"check\", round, attempts FROM agents WHERE scope = ?",
        ).all(p.scope,);
...
function apiScope(sessionId: string, scope: string,): object | null
{
  if (!SESSION_DIR_RE.test(sessionId,))
  {
    return null;
...
function apiArtifact(sessionId: string, scope: string, agent: string,): string | null
{
  if (!SESSION_DIR_RE.test(sessionId,))
  {
    return null;
