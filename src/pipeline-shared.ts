 ...
 */
function extractFrontmatter(mdContent: string): string | null {
 ...
 */
function parseVerification(mdContent: string): VerificationStep[] | null {
 ...
 */
function parseStepArray(inner: string): VerificationStep | null {
 ...
 */
function parseConditions(mdContent: string): string | null {
  const fm = extractFrontmatter(mdContent);
  if (!fm) return null;
 ...
 */
function requiresScope(mdContent: string): boolean {
  const fm = extractFrontmatter(mdContent);
  if (!fm) return false;
 ...
 */
function findAgentMd(agentType: string, projectRoot: string | null, home: string | null): string | null {
  if (projectRoot) {
 ...
 */
function agentRunningMarker(sessionDir: string, scope: string): string {
  return path.join(sessionDir, `.running-${scope}`).replace(/\\/g, "/");
}

export { extractFrontmatter, parseVerification, parseConditions, requiresScope, findAgentMd, getSessionDir, agentRunningMarker, VERDICT_RE };
