...
test("full flow: parse agent → create pipeline → step through", () => {
    ...
    assert_1.default.strictEqual(steps[1].type, "REVIEW");
...
test("full flow: role resolution through pipeline lifecycle", () => {
    ...
    try {
        ...
        assert_1.default.strictEqual(engine.resolveRole(db, "role-scope", "e2e-reviewer"), "ungated"); // step 0 is SEMANTIC, not REVIEW
...
test("verdict file: write → block hook reads → advances → deletes", () => {
    ...
    try {
        ...
        assert_1.default.strictEqual(a1.action, "command");
        // Write verdict file (simulating /pass_or_revise skill)
        const scopeDir = path_1.default.join(dir, "cmd-scope");
        fs_1.default.mkdirSync(scopeDir, { recursive: true });
        const verdictPath = path_1.default.join(scopeDir, ".command-verdict.md");
        fs_1.default.writeFileSync(verdictPath, "Result: PASS\n");
        ...
        assert_1.default.ok(match, "VERDICT_RE should match");
        assert_1.default.strictEqual(match[1], "PASS");
        ...
        assert_1.default.strictEqual(crud.getStep(db, "cmd-scope", 0).status, "passed");
        assert_1.default.strictEqual(crud.getStep(db, "cmd-scope", 1).status, "active");
        assert_1.default.ok(!fs_1.default.existsSync(verdictPath), "Verdict file should be deleted");
    }
    finally {
        db.close();
        cleanup(dir);
...
test("verdict file: REVISE sends back to source", () => {
    ...
    try {
        ...
        assert_1.default.strictEqual(crud.getPipelineState(db, "rev-cmd").status, "revision");
    }
    finally {
        db.close();
        cleanup(dir);
...
test("source REVISE advances SEMANTIC (source doesn't judge itself)", () => {
    ...
    try {
        ...
        assert_1.default.strictEqual(a.action, "spawn", "Should advance to REVIEW step");
...
test("gt-worker verification: field parses to SEMANTIC + REVIEW_WITH_FIXER", () => {
    ...
    assert_1.default.strictEqual(steps[1].type, "REVIEW_WITH_FIXER");
...
test("gt-worker requiresScope returns true", () => {
    const agentMd = fs_1.default.readFileSync(path_1.default.join(process.cwd(), ".claude", "agents", "gt-worker.md"), "utf-8");
    assert_1.default.strictEqual(shared.requiresScope(agentMd), true);
});
// ══════════════════════════════════════════════════════════════════════
// Cleanup
cleanup(tempRoot);
...
//# sourceMappingURL=test-pipeline-e2e.js.map